...
sequenceDiagram
    ...
    User->>SingleVPS: git push (triggers GHA build & deploy)
...
-   **Deployment**: Uses **GitHub Actions** to build the image and push to GHCR. The VPS pulls the optimized image, preventing OOM crashes on low-memory hardware.
