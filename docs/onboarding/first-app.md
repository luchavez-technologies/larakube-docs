...
Ready to bring your first masterpiece to the cluster? LaraKube CLI makes the process as intuitive as running a standard Laravel installer.
...
> 💡 **Fast Track:** Want to skip the LaraKube CLI questions and use our "ideal" high-performance defaults (MySQL, Redis, Reverb, etc.)? Just add the `--fast` flag:
> `larakube new my-awesome-app --fast`

## 2. Architectural Choices
LaraKube CLI will guide you through several thoughtful decisions:
...
LaraKube CLI will automatically:
...
LaraKube CLI makes daily development fast and stable. Instead of deleting your environment when you're finished for the day, use the **Stop/Start** workflow to preserve your data:
