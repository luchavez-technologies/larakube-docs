...
When you enable Octane, LaraKube CLI configures your application to stay in memory:
...
## 🏛 Required Engine
To use Octane in LaraKube CLI, you must select **FrankenPHP** as your server variation. LaraKube CLI will then automatically:
...
Ensure your `OCTANE_SERVER` is set to `frankenphp` in your `.env`. LaraKube CLI handles this automatically during project initialization.
