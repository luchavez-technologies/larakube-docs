...
Usually, you don't even need to edit the blueprint manually. When you run `larakube cloud:configure` to record a multi-node managed cluster (like DOKS or EKS), LaraKube automatically detects the cluster's node count and switches your strategy to `multi-node-ha` for you.
...
- **Redundancy**: LaraKube increases pod replica counts and configures pod anti-affinity to ensure services are strictly distributed across your new nodes for true high availability.
